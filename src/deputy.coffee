...
deputyOptions =
  ...
  defaultResourceQueryGenerator: (model) ->
    (param) -> id: param
  ...
  defaultIndexQueryGenerator: (model, indexDef) ->
    props = indexDef.properties
    (param) ->
      if props.length == 1
        query = {}
  ...
  defaultQueryIdExtractor: (model) ->
    (query={}) -> query[model.id]
  defaultQueryIndexKeyExtractor: (model, indexDef) ->
    props = indexDef.properties
    (query={}) ->
      props.map (p) -> query[p]

  generateHasManyServiceName: (propertyName, modelName) ->
  ...
  generateHasManyEndpointName: (propertyName, modelName) ->
    'findBy'+modelName[0].toUpperCase()+modelName.substring(1)+'Id'

  generateBelongsToServiceName: (propertyName, modelName) ->
    idless = propertyName.substring(0, propertyName.length-2)
    idless[0].toUpperCase() + idless.substring(1)
  generateBelongsToEndpointName: (propertyName, modelName) ->
    'get'
...
mod.provider 'Deputy', -> angular.extend(deputyOptions, $get: Deputy)
...
Deputy = [
  ...
  ($http,   $rootScope,   $q,   DeputyModel,   DeputyStore) ->
    ...
    #   }
    # }
    normalizeSelector = (select, model) ->
    ...
    generateEndpoint = (name, defn, defaults, store) ->
      ...
      Deputy[action](store, defn)

    deputy = class Deputy
      constructor: (name, options={}) ->
        if !options.endpoints
          throw new Error("new Deputy: you must provide endpoints")
    ...
    Deputy.get = (store, actionOptions) ->
      model = store.model

      if actionOptions.embedded
        for reference in actionOptions.embedded
          if !model.references[reference]
            throw new Error("Deputy.get: unknown reference specified in `embedded`")
        ...
        $http(
          ...
          if requestedId
            if !receivedId
              ...
              throw new Error("Deputy.get: fetched data no longer exists")
            else if requestedId != receivedId
              throw new Error("Deputy.get: endpoint returned wrong id")
          else if !receivedId
            throw new Error("Deputy.get: fetch did not return id")
          else
            receivedId
        ...
        if outOfDate.length > 0
          $q.all refNames.map (refName) ->
            ...
            if (Object.keys(endpointOptions.query).length == 0 and
              ...
              throw new Error("Deputy.get: empty query/id for selected reference")
        ...
        promise = if isRootFetchRequired
          ...
          fetchRoot(requestedId, query, options.body).then (receivedId) ->
            ...
            if storeWindow.resource.$getOutOfDate(rootSelected, options.maxAge).length > 0
              throw new Error("Deputy.get: selected data could not be fetched")
    ...
    #     object
    Deputy.index = (store, actionOptions={}) ->
      ...
      if actionOptions.embedded
        for reference in actionOptions.embedded
          if !model.references[reference]
            throw new Error("Deputy.index: unknown reference specified in `embedded`")

      if !actionOptions.indexName
        throw new Error("Deputy.index: `indexName` is required!")
      ...
      (options={}) ->
        ...
        if !requestedIndexKey
          throw new Error("Deputy.index: no index key can be read from query. Consider using list or partiallist instead.")
        ...
        for refName in references
          ...
          if refName not in actionOptions.embedded
            throw new Error("Deputy.index: reference #{refName} is not embedded, and so can't be selected")
    ...
    Deputy.list = (store, actionOptions={}) ->
      # TODO:
    ...
    Deputy.partiallist = (store, actionOptions={}) ->

    # Creates endpoints for creating a resource or list of resources
    Deputy.post = (store, actionOptions={}) ->
      # TODO:
    ...
    #       receive a result.
    Deputy.delete = (store, actionOptions={}) ->
      (options={}) ->
        # TODO:
    ...
    Deputy.patch = (store, actionOptions={}) ->
      # TODO:
